...
lst_animals = list(filter(lambda element: issubclass(type(element), Animals), lst_objs))  # все объекты, относящиеся к животным (Animals);
lst_plants = list(filter(lambda element: issubclass(type(element), Plants), lst_objs))  # все объекты, относящиеся к растениям (Plants);
lst_mammals = list(filter(lambda element: issubclass(type(element), Mammals), lst_objs))  # все объекты, относящиеся к млекопитающим (Mammals).

[print(i.name) for i in lst_animals]