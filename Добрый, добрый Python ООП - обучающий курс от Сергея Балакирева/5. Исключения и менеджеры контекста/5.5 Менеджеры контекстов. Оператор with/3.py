...
class Box:
    ...
    def add_thing(self, obj):
        if self.current_weight() + obj[1] > self._max_weight:
            raise ValueError('превышен суммарный вес вещей')
...
class BoxDefender:
    ...
    def __enter__(self):
        self.list = Box(self.box._name, self.box._max_weight)
        self.list._things = self.box._things[:]
        return self.list

    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type is None:
            self.box._things = self.list._things
        return False
