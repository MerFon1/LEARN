...
class Bag:
    ...
    def add_thing(self, thing):
        if thing.weight<=self.max_weight-self.busy_weight():
            self.bag_pag.append(thing)
        else:
            raise ValueError('превышен суммарный вес предметов')

    def __getitem__(self, item):
        if self.bag_pag[item].weight<=self.max_weight-self.busy_weight():
            return self.bag_pag[item]
        raise ValueError('превышен суммарный вес предметов')

    def __setitem__(self, key, value):
        if self.bag_pag[key].weight<=self.max_weight-self.busy_weight():
            self.bag_pag[key] = value
        else:
            raise ValueError('превышен суммарный вес предметов')


    def __delitem__(self, key):
        del self.bag_pag[key]


    def busy_weight(self):
        if len(self.bag_pag)==0:
            return 0
        result = 0
        for i in self.bag_pag:
            result += i.weight
        return result
