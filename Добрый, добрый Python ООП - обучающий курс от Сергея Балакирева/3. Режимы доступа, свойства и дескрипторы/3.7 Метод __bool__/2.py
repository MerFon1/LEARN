...
class MailBox:
    ...
    def receive(self):
        self.inbox_list = list(map(lambda m: MailItem(*m.split('; ')), list(map(str.strip, sys.stdin.readlines()))))
...
mail = MailBox()
mail.receive()
mail()[0].set_read(True)
mail()[-1].set_read(True)
inbox_list_filtered = list(filter(lambda m: bool(m), mail()))
